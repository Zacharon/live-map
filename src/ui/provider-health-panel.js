...
export function renderProviderHealthPanel(sourceStatus = {}, providerResults = []) {
  ...
  const rows = Object.entries(sourceStatus).map(([providerId, status]) => {
    ...
    const recordKinds = Object.entries(status.recordKinds || result.recordKinds || {}).map(([kind, count]) => `${kind}: ${count}`).join(", ") || "events only";
    const nextRefresh = status.nextScheduledRefreshAt ? relativeTime(new Date(status.nextScheduledRefreshAt).getTime()) : "unknown";
    return `<article class="provider-row provider-${state}"><div><strong>${escapeHtml(providerId.toUpperCase())}</strong><span>${escapeHtml(state)}</span></div><dl><dt>Last attempt</dt><dd>${escapeHtml(lastAttempt)}</dd><dt>Last success</dt><dd>${escapeHtml(lastSuccess)}</dd><dt>Latest source event</dt><dd>${escapeHtml(mostRecent)}</dd><dt>Duration</dt><dd>${escapeHtml(status.durationMs ?? result.durationMs ?? "-")}ms</dd><dt>Received</dt><dd>${escapeHtml(result.recordCount ?? status.count ?? 0)}</dd><dt>Accepted</dt><dd>${escapeHtml(status.acceptedCount ?? status.count ?? 0)}</dd><dt>Rejected</dt><dd>${escapeHtml(status.rejectedCount ?? result.rejectedCount ?? 0)}</dd><dt>Duplicates</dt><dd>${escapeHtml(result.duplicateCount ?? status.duplicateCount ?? 0)}</dd><dt>Clusters</dt><dd>${escapeHtml(status.recordsClustered ?? result.recordsClustered ?? 0)}</dd><dt>Promoted</dt><dd>${escapeHtml(status.recordsPromoted ?? result.recordsPromoted ?? 0)}</dd><dt>Record kinds</dt><dd>${escapeHtml(recordKinds)}</dd><dt>Next refresh</dt><dd>${escapeHtml(nextRefresh)}</dd><dt>State store</dt><dd>${escapeHtml(status.stateStorageMode || result.stateStorageMode || "memory")}</dd><dt>Cache</dt><dd>${status.stale ? `stale ${escapeHtml(cacheAge)}` : "fresh"}</dd></dl><p>${escapeHtml(status.message || "No provider message.")}</p>${status.url ? `<a href="${escapeHtml(status.url)}" target="_blank" rel="noopener noreferrer">Provider documentation</a>` : ""}<button type="button" data-provider-retry="${escapeHtml(providerId)}">Retry</button></article>`;
  });
