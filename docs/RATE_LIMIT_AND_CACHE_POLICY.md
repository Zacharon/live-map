...
Cloudflare Worker `/api/*` routes and Netlify compatibility functions use a lightweight fixed-window limiter in `src/api/rate-limit.js`.
...
- Oversized request behavior: `/api/*` requests with `Content-Length` above 16 KiB are rejected with JSON `413` before route handling. Netlify JSON POST compatibility routes also stream request bodies with the same byte cap and return JSON `400` for malformed JSON.
...
This is best-effort per Worker isolate or Netlify function instance. It is intentionally Free-compatible and does not claim globally consistent enforcement across the Cloudflare edge or Netlify runtime. If the project later adopts Cloudflare-native Rate Limiting bindings or WAF rate limiting rules, keep those controls optional and documented rather than required defaults.
